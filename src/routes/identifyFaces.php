...
$app->post('/api/MicrosoftFaceApi/identifyFaces', function ($request, $response, $args) {
    ...
    if(!empty($post_data['args']['confidenceThreshold'])) {
        $body['confidenceThreshold'] = $post_data['args']['confidenceThreshold'];
    }
    
    $headers['Ocp-Apim-Subscription-Key'] = $post_data['args']['subscriptionKey'];
    $headers['Content-Type'] = 'application/json';
     if(!empty($post_data['args']['region'])){         $settings['api_url'] = "https://".$post_data['args']['region'].".api.cognitive.microsoft.com/face/v1.0/";     }  $query_str = $settings['api_url'] . 'identify';
